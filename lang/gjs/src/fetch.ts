const promiseTask = (session, msg) => {
  return new Promise((resolve, reject) => {
    session.send_async(msg, null, null, (_self, asyncResult) => {
      try {
        resolve(session.send_finish(asyncResult));
      } catch (err) {
        reject(err);
      }
    });
...
export default async function fetch(url, options = {}) {
  ...
  const inputStream = await promiseTask(session, message);
